...
#define PWM_FREQUENCY 15000 		//PWM频率16K, 将被计算出pwm值,写入PWMPH/L
...
static unsigned char comCounter = 0; //换相计数，用于计算PID
     	...
     	zOk = 0;                 // 重置为未过0 // 只有在没有过0时，才会启用ADC；当过0后，还未换相前不启用ADC
				...
				else if (adcSwitch == 1) 			
        		{
					...
					if ( zOk == 1) {
						...
						clr_TR1;            // 停止term1
						//t1pv = 0xFFFF - (cv/3.2);                // set timer1 preValue
						//t1pv = 0xFFFF - ((cv + cv1+ cv2)/6);                // set timer1 preValue
						//t1pv = 0xFFFF - (cv2/2.5);                // set timer1 preValue
						//t1pv = 0xFFFF - (cv/3.2);                // set timer1 preValue
						//t1pv = 0xFFFF - (cv/3.8);                // set timer1 preValue
